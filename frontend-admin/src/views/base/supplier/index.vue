<template>
  <div class="page-container">
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'action'">
            <a-space>
              ...
              <a-popconfirm title="确定要删除该供应商吗？" @confirm="handleDelete(record)">
    ...
    <!-- 编辑弹窗 -->
    <a-modal v-model:open="dialogVisible" :title="dialogTitle" @ok="handleSubmit" :confirm-loading="submitLoading" width="750px">
      <a-form ref="formRef" :model="form" :rules="rules" :label-col="{ span: 7 }" :wrapper-col="{ span: 16 }" style="padding: 16px 0;">
        <a-row :gutter="24">
          <a-col :span="12">
...
import { SearchOutlined, ReloadOutlined, PlusOutlined, ContactsOutlined } from '@ant-design/icons-vue'
import { getSupplierPage, saveSupplier, updateSupplier, deleteSupplier } from '@/api/base'
...
const searchForm = reactive({ supplierName: '', status: null })
const pagination = reactive({ current: 1, pageSize: 10, total: 0, showSizeChanger: true, showTotal: t => `共 ${t} 条` })
const form = reactive({ id: null, supplierCode: '', supplierName: '', contact: '', phone: '', address: '', remark: '', status: 1 })
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await getSupplierPage({ page: pagination.current, size: pagination.pageSize, ...searchForm })
    tableData.value = res.data.records
    pagination.total = res.data.total
  } catch (e) { logger.error('Failed to load supplier list', { error: e.message }) }
...
const handleAdd = () => {
  ...
  Object.assign(form, { id: null, supplierCode: '', supplierName: '', contact: '', phone: '', address: '', remark: '', status: 1 })
...
const handleSubmit = async () => {
  try {
    ...
    logger.error('Failed to save supplier', { error: e.message })
  } finally {
...
const handleDelete = async (record) => {
  try {
    ...
    logger.error('Failed to delete supplier', { error: e.message })
  }
}
