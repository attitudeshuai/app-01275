          ...
          <a-col :span="8">
            <a-form-item label="供应商" name="supplierId">
              <a-select
                ...
                :field-names="{ label: 'supplierName', value: 'id' }"
              />
            </a-form-item>
          </a-col>
          <a-col :span="8">
...
import { getSupplierList, getWarehouseList } from '@/api/base'
import { applyPurchase } from '@/api/purchase'
...
const filterOption = (input, option) => option.supplierName.toLowerCase().includes(input.toLowerCase())
...
const fetchSupplierList = async () => {
  try {
    ...
    logger.error('Failed to load supplier list', { error: e.message })
  }
}
...
const handleSubmit = async () => {
  try {
    ...
    await saveGoodsHistory('PURCHASE', form.items.map(i => i.goodsId))
    message.success('采购申请提交成功')
    logger.info('Purchase apply submitted')
    router.push('/purchase/order')
  } catch (e) {
    if (e.errorFields) return
    logger.error('Failed to submit purchase apply', { error: e.message })
  } finally {
